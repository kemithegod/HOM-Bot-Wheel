...
let dommes = fs.existsSync('dommes.json')
  ...
  : [
    ...
    'Princess Myiaa', 'Demoness Maria', 'Miss Feetus', 'Goddess Kemi', 'Black Widow', 'TT', 'Kitty Darling', 'Princess Veyra'
  ];
...
function getPaymentLinks(domme) {
  if (cachedPayments && cachedPayments[domme]) {
    const val = cachedPayments[domme];
    // Support both old single string format and new array format
    if (Array.isArray(val)) return val;
    return [{ url: val, label: 'Send tribute' }];
  }
  return [];
}
...
client.once('ready', async () => {
  ...
  const commands = [
    ...
    new SlashCommandBuilder()
      .setName('addpayment')
      .setDescription('Add a payment link for a domme (supports multiple)')
      .addStringOption(o => o
        .setName('name')
      ...
      .addStringOption(o => o
        .setName('label')
        .setDescription('Button label e.g. CashApp, Throne, Venmo (optional)')
        .setRequired(false)),
    ...
    new SlashCommandBuilder()
      ...
      .addStringOption(o => o
        .setName('name')
        .setDescription('Domme name')
        .setRequired(true)
        .setAutocomplete(true))
      .addIntegerOption(o => o
        .setName('number')
        .setDescription('Link number to remove (use /listpayments to see numbers)')
        .setRequired(true)),
...
client.on('interactionCreate', async interaction => {
        ...
        await dommeUser.send({
          ...
          ...(paymentLinks.length > 0 ? {
            components: [new ActionRowBuilder().addComponents(
              ...paymentLinks.map(p => new ButtonBuilder()
                .setLabel(p.label || 'Your tribute link')
                .setURL(p.url)
                .setStyle(ButtonStyle.Link)
                .setEmoji('💸')
              )
  ...
  if (interaction.commandName === 'listdommes') {
    await interaction.reply({
      embeds: [new EmbedBuilder()
        ...
        .setDescription(dommes.map((d, i) => { const hasDMs = dommeRegistry[d]; const hasPay = cachedPayments && cachedPayments[d] && (Array.isArray(cachedPayments[d]) ? cachedPayments[d].length > 0 : true); return `${i + 1}. ${d}${hasDMs ? ' ✅' : ''}${hasPay ? ' 💸' : ''}`; }).join('\n'))
        .setColor(0x6a0dad)
  ...
  if (interaction.commandName === 'addpayment') {
    ...
    // Convert old single string to array if needed
    if (typeof cachedPayments[name] === 'string') {
      cachedPayments[name] = [{ url: cachedPayments[name], label: 'Send tribute' }];
    }
    if (!Array.isArray(cachedPayments[name])) cachedPayments[name] = [];
    cachedPayments[name].push({ url: link, label });
    await savePayments();
  ...
  if (interaction.commandName === 'removepayment') {
    ...
    await savePayments();
    await interaction.reply({ content: `🗑️ Payment link #${num} removed for **${name}**.`, ephemeral: true });
  }
  ...
  if (interaction.commandName === 'listpayments') {
    ...
    const links = Array.isArray(cachedPayments[name]) ? cachedPayments[name] : [{ url: cachedPayments[name], label: 'Send tribute' }];
    const list = links.map((p, i) => (i+1) + '. ' + p.label + ' -> ' + p.url).join('\n');
