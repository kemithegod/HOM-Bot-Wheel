...
client.once('ready', async () => {
  ...
  const commands = [
    new SlashCommandBuilder()
      ...
      .addIntegerOption(o => o.setName('min').setDescription('Minimum amount (default $33)').setMinValue(DEFAULT_MIN).setMaxValue(DEFAULT_MAX).setRequired(false))
      .addIntegerOption(o => o.setName('max').setDescription('Maximum amount (default $777)').setMinValue(DEFAULT_MIN).setMaxValue(DEFAULT_MAX).setRequired(false)),
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'spin') {
    ...
    if ((minVal === maxVal && minVal < 100) || (minVal > maxVal) || (minVal !== maxVal && (maxVal - minVal) < 15)) {
      return interaction.reply({ content: 'Invalid range. Same min/max only allowed for $100+. Otherwise must be at least $15 apart.', ephemeral: true });
    }
